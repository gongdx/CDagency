...
class CdShengtaijuSpider(scrapy.Spider):
    name = 'CD18shengtaiju'
    allowed_domains = ['sthj.chengdu.gov.cn']
    start_urls = ['http://sthj.chengdu.gov.cn/search/s?q=1&qt=%E4%B8%8D%E5%BF%98%E5%88%9D%E5%BF%83%E3%80%81%E7%89%A2%E8%AE%B0%E4%BD%BF%E5%91%BD&pageSize=10&sort=D&database=all&siteCode=5101000012&docQt=&page=1']
    ...
    def parse(self, response):
        ...
        next_url = 'http://sthj.chengdu.gov.cn/search/s?q=1&qt=%E4%B8%8D%E5%BF%98%E5%88%9D%E5%BF%83%E3%80%81%E7%89%A2%E8%AE%B0%E4%BD%BF%E5%91%BD&pageSize=10&sort=D&database=all&siteCode=5101000012&docQt=&page=' + str(int(num)+1)
        if int(num) < 15:
    ...
    def get_text(self, response):
        ...
        item = response.meta['item']
        # 获取详细内容
        content_list = response.xpath('//div[@class="conts"]//text()').getall()
        ...
        # department = response.xpath('//div[@class="set"]/text()').get()
