...
class CdKouanwuliujuSpider(scrapy.Spider):

    name = 'CD22kouanwuliuju'
    allowed_domains = ['cdwl.chengdu.gov.cn']
    start_urls = ['http://cdwl.chengdu.gov.cn/search/s?q=1&qt=%E4%B8%8D%E5%BF%98%E5%88%9D%E5%BF%83%E3%80%81%E7%89%A2%E8%AE%B0%E4%BD%BF%E5%91%BD&pageSize=10&sort=D&database=all&siteCode=5101000039&docQt=&page=1']
    ...
    def parse(self, response):
        ...
        for new in news:
            ...
            item = CDagency()
            item["col_name"] = "CD22kouanwuliuju"
            ...
            # print(href)
            initial_url = 'http://cdwl.chengdu.gov.cn/search/' + str(href)
        ...
        next_url = 'http://cdwl.chengdu.gov.cn/search/s?q=1&qt=%E4%B8%8D%E5%BF%98%E5%88%9D%E5%BF%83%E3%80%81%E7%89%A2%E8%AE%B0%E4%BD%BF%E5%91%BD&pageSize=10&sort=D&database=all&siteCode=5101000039&docQt=&page=' + str(int(num)+1)
        if int(num) < 15:
    ...
    def get_detail_url(self, response):
        ...
        s = re.search('location.href\ =\ "(.*?)";', response.text).group(1)
        detail_url = 'http://cdwl.chengdu.gov.cn' + s
        item['detail_url'] = detail_url
    ...
    def get_text(self, response):
        ...
        # 获取详细内容
        content_list = response.xpath('//div[@class="content"]//text()').getall()
