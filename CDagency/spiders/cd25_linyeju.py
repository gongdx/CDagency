...
class CdLinyejuSpider(scrapy.Spider):
    name = 'CD25linyeju'
    allowed_domains = ['cdbpw.chengdu.gov.cn']
    start_urls = ['http://cdbpw.chengdu.gov.cn/search/s?q=1&qt=%E4%B8%8D%E5%BF%98%E5%88%9D%E5%BF%83%E3%80%81%E7%89%A2%E8%AE%B0%E4%BD%BF%E5%91%BD&pageSize=10&sort=D&database=all&siteCode=5101000044&docQt=&page=1']
    ...
    def parse(self, response):
        ...
        for new in news:
            ...
            initial_url = 'http://cdfao.chengdu.gov.cn/search/' + str(href)
        ...
        next_url = 'http://cdbpw.chengdu.gov.cn/search/s?q=1&qt=%E4%B8%8D%E5%BF%98%E5%88%9D%E5%BF%83%E3%80%81%E7%89%A2%E8%AE%B0%E4%BD%BF%E5%91%BD&pageSize=10&sort=D&database=all&siteCode=5101000044&docQt=&page=' + str(int(num)+1)
        if int(num) < 15:
    ...
    def get_text(self, response):
        ...
        item = response.meta['item']
        # 获取详细内容
        content_list = response.xpath('//td[@class="content2"]//text()').getall()
        ...
        # 获取部门信息
        # department = response.xpath('//div[@class="bar4"]/span[2]/text()').get()
        # item['department'] = department

        # 获取浏览量
        # browse_times = response.xpath('//div[@class="bar4"]/span[@id="Browse"]/text()').get()
