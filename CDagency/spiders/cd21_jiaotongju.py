...
class CdJiaotongjuSpider(scrapy.Spider):
    name = 'CD21jiaotongju'
    allowed_domains = ['jtys.chengdu.gov.cn']
    start_urls = ['http://jtys.chengdu.gov.cn/search/s?q=1&qt=%E4%B8%8D%E5%BF%98%E5%88%9D%E5%BF%83%E3%80%81%E7%89%A2%E8%AE%B0%E4%BD%BF%E5%91%BD&pageSize=10&sort=D&database=all&siteCode=5101000070&docQt=&page=1']
    ...
    def parse(self, response):
        ...
        for new in news:
            ...
            initial_url = 'http://cdfao.chengdu.gov.cn/search/' + href
        ...
        next_url = 'http://jtys.chengdu.gov.cn/search/s?q=1&qt=%E4%B8%8D%E5%BF%98%E5%88%9D%E5%BF%83%E3%80%81%E7%89%A2%E8%AE%B0%E4%BD%BF%E5%91%BD&pageSize=10&sort=D&database=all&siteCode=5101000070&docQt=&page=' + str(int(num)+1)
        if int(num) < 15:
    ...
    def get_text(self, response):
        ...
        item = response.meta['item']
        # 获取详细内容
        content_list = response.xpath('//div[@id="article_content"]//text()').getall()
        ...
        # department = response.xpath('//div[@class="article_info"]/text()').get()
