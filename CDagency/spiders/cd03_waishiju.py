...
class CdWaishijuSpider(scrapy.Spider):
    ...
    name = 'CD03waishiju'
    allowed_domains = ['cdfao.chengdu.gov.cn']
    start_urls = ['http://cdfao.chengdu.gov.cn/search/s?q=1&qt=不忘初心+牢记使命&pageSize=10&sort=D&database=all&siteCode=5101000006&docQt=&page=1']
    ...
    def parse(self, response):
        ...
        next_url = 'http://cdfao.chengdu.gov.cn/search/s?q=1&qt=不忘初心+牢记使命&pageSize=10&sort=D&database=all&siteCode=5101000006&docQt=&page=' + str(int(num)+1)
        if int(num) < 15:
    ...
    def get_text(self, response):
        ...
        # 获取详细内容
        content_list = response.xpath('//td[@align="left" and @style="font-size:14px; line-height:180%;"]//text()').getall()
