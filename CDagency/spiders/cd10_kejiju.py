...
class CdKejijuSpider(scrapy.Spider):
    name = 'CD10kejiju'
    allowed_domains = ['cdst.chengdu.gov.cn']
    start_urls = ['http://cdst.chengdu.gov.cn/search/s?q=1&qt=不忘初心、牢记使命&pageSize=10&sort=D&database=all&siteCode=5101000050&docQt=&page=1']
    ...
    def parse(self, response):
        ...
        for new in news:
            ...
            # print(href)
            initial_url = 'http://cdst.chengdu.gov.cn/search/' + str(href)
            # print(detail_url)
        ...
        next_url = 'http://cdst.chengdu.gov.cn/search/s?q=1&qt=不忘初心、牢记使命&pageSize=10&sort=D&database=all&siteCode=5101000050&docQt=&page=' + str(int(num)+1)
        if int(num) < 15:
    ...
    def get_text(self, response):
        ...
        content_list = response.xpath('//div[@class="content" and @id="content"]//text()').extract()
        # 对文本进行拼接
