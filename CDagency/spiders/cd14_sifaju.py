...
class CdSifajuSpider(scrapy.Spider):
    name = 'CD14sifaju'
    allowed_domains = ['cdjustice.chengdu.gov.cn']
    start_urls = ['http://cdjustice.chengdu.gov.cn/search/s?q=1&qt=%E4%B8%8D%E5%BF%98%E5%88%9D%E5%BF%83%E3%80%81%E7%89%A2%E8%AE%B0%E4%BD%BF%E5%91%BD&pageSize=10&sort=D&database=all&siteCode=5101000046&docQt=&page=1']
    ...
    def parse(self, response):
        ...
        for new in news:
            item = CDagency()
            item["col_name"] = "CD14sifaju"

            href = new.xpath('.//a[@class="link"]/@href').extract_first()
            initial_url = 'http://cdjustice.chengdu.gov.cn/search/' + str(href)
        ...
        next_url = 'http://cdjustice.chengdu.gov.cn/search/s?q=1&qt=%E4%B8%8D%E5%BF%98%E5%88%9D%E5%BF%83%E3%80%81%E7%89%A2%E8%AE%B0%E4%BD%BF%E5%91%BD&pageSize=10&sort=D&database=all&siteCode=5101000046&docQt=&page=' + str(int(num)+1)
        if int(num) < 15:
    ...
    def get_text(self, response):
        ...
        content_list = response.xpath('//*[@id="zoomcon" or @id="myFont"]//text()').extract()
        # 对文本进行拼接
