...
class CdfagaiweiSpider(scrapy.Spider):
    name = 'CD07fagaiwei'
    allowed_domains = ['cddrc.chengdu.gov.cn']
    start_urls = ['http://cddrc.chengdu.gov.cn/search/s?q=1&qt=不忘初心、牢记使命&pageSize=10&sort=D&database=all&siteCode=5101000007&docQt=&page=1']
    ...
    def parse(self, response):
        ...
        for new in news:
            ...
            # print(href)
            initial_url = 'http://cddrc.chengdu.gov.cn/search/' + str(href)
        ...
        next_url = 'http://cddrc.chengdu.gov.cn/search/s?q=1&qt=不忘初心、牢记使命&pageSize=10&sort=D&database=all&siteCode=5101000007&docQt=&page=' + str(int(num)+1)
        if int(num) < 15:
    ...
    def get_text(self, response):
        ...
        content_list = response.xpath('//div[@class="content" or @class="content-area"]//text()').extract()
        # 对文本进行拼接
